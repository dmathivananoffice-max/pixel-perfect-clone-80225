...
RETURNS text LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT role_key FROM public.app_users WHERE auth_user_id = auth.uid() AND active = true LIMIT 1
$$;

CREATE OR REPLACE FUNCTION public.has_role(_role_key text)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.app_users WHERE auth_user_id = auth.uid() AND active = true AND role_key = _role_key)
$$;

CREATE OR REPLACE FUNCTION public.has_any_role(_role_keys text[])
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.app_users WHERE auth_user_id = auth.uid() AND active = true AND role_key = ANY(_role_keys))
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.has_any_role(ARRAY['super_admin','managing_director','operations_manager','sales_executive','recruiter','documentation_officer','german_trainer']);
$$;

CREATE OR REPLACE FUNCTION public.handle_new_auth_user()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _role text; _name text;
BEGIN
  IF NEW.email = 'deeban@workforce-europe.com' THEN _role := 'super_admin'; ELSE _role := 'candidate'; END IF;
  ...
  VALUES (NEW.id, NEW.email, _name, _role, _role = 'super_admin')
...
BEGIN
  IF TG_OP = 'DELETE' AND OLD.role_key = 'super_admin' THEN
    SELECT count(*) INTO _remaining FROM public.app_users WHERE role_key='super_admin' AND active=true AND id <> OLD.id;
    IF _remaining = 0 THEN RAISE EXCEPTION 'Cannot delete the last active Super Administrator'; END IF;
  ELSIF TG_OP = 'UPDATE' AND OLD.role_key = 'super_admin' AND (NEW.role_key <> 'super_admin' OR NEW.active = false) THEN
    SELECT count(*) INTO _remaining FROM public.app_users WHERE role_key='super_admin' AND active=true AND id <> OLD.id;
    IF _remaining = 0 THEN RAISE EXCEPTION 'Cannot demote or deactivate the last Super Administrator'; END IF;
  END IF;
...
CREATE TRIGGER trg_protect_last_super_admin BEFORE UPDATE OR DELETE ON public.app_users
...
CREATE POLICY "admins insert" ON public.app_users FOR INSERT TO authenticated WITH CHECK (public.is_admin());
CREATE POLICY "admins update" ON public.app_users FOR UPDATE TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "admins delete" ON public.app_users FOR DELETE TO authenticated USING (public.is_admin());
...
CREATE POLICY "authenticated append audit" ON public.audit_events FOR INSERT TO authenticated
...
BEGIN
  FOR r IN SELECT policyname FROM pg_policies WHERE schemaname='storage' AND tablename='objects'
    AND (policyname ILIKE '%candidate-documents%' OR policyname ILIKE '%candidate docs%') LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON storage.objects', r.policyname);
  END LOOP;
END $$;

CREATE POLICY "candidate-documents staff read" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'candidate-documents' AND public.is_staff());
CREATE POLICY "candidate-documents staff insert" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'candidate-documents' AND public.is_staff());
CREATE POLICY "candidate-documents staff update" ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'candidate-documents' AND public.is_staff())
  WITH CHECK (bucket_id = 'candidate-documents' AND public.is_staff());
CREATE POLICY "candidate-documents admin delete" ON storage.objects FOR DELETE TO authenticated
