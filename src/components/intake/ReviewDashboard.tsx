...
export function ReviewDashboard({
  ...
  const cards: {
    ...
    },
    {
      ...
      hint: "One or more docs not found",
      tone: "text-rose-700",
      icon: FileX2,
    },
    {
      id: "manual_review",
      label: "Manual review",
      hint: "AI could not extract a field",
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
              <span>
                Product · <span className="font-medium text-foreground">{batch.productLabel}</span>
              </span>
              <span aria-hidden>·</span>
              <span>
                {total} candidate{total === 1 ? "" : "s"} uploaded
              </span>
              <span aria-hidden>·</span>
                ...
                return (
                  <div key={c.id} className="rounded-md">
                    ...
                    >
                      ...
                      <span
                        className={cn(
                          "ml-auto rounded-full px-1.5 py-0.5 text-[10px] font-medium ring-1",
                      ...
                      <ul className="ml-6 border-l border-border/60 pl-2">
                        {c.documents.map((d) => (
                          <li key={d.key}>
                            <button
                              onClick={() =>
                                onVerify(c.id, { section: "documents", fieldKey: d.key })
                              }
                              className={cn(
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <div className={cn("font-display text-2xl font-semibold tabular-nums", tone)}>
            {value}
          </div>
        </div>
  ...
  return (
    <div className="rounded-xl border border-amber-200 bg-amber-50/40">
      ...
      <div className="max-h-[420px] divide-y divide-amber-200/60 overflow-y-auto">
        {duplicates.length === 0 && (
          <div className="p-6 text-center text-sm text-amber-900/80">
            No duplicates detected. Nice batch.
          </div>
        )}
        {duplicates.map((c) => {
              ...
              <div className="grid grid-cols-[minmax(0,80px)_1fr_1fr] gap-x-3 gap-y-1 rounded-lg bg-background/60 p-3 text-xs">
                ...
                {rows.map(([label, a, b]) => (
                  <div key={label} className="contents">
                    ...
                    <div className={cn("truncate", a === b && "text-rose-700 font-medium")}>
                      {a}
                    </div>
                    <div className={cn("truncate", a === b && "text-rose-700 font-medium")}>
                      {b}
...
function IssueCentre({
  ...
  const toneClass =
    tone === "rose" ? "border-rose-200 bg-rose-50/40" : "border-violet-200 bg-violet-50/40";
  ...
  return (
    <div className={cn("rounded-xl border", toneClass)}>
      ...
      <ul className="max-h-[260px] divide-y divide-border/40 overflow-y-auto">
        {items.length === 0 && (
          <li className="p-6 text-center text-sm text-muted-foreground">{empty}</li>
        )}
        {items.map((it) => (
