...
export function DocumentFingerprintPanel({
  candidateId,
  hideHeader,
}: DocumentFingerprintPanelProps) {
  ...
  return (
    <div className="space-y-3 p-3">
      ...
      {loading && (
        <div className="text-xs text-muted-foreground">Loading document fingerprints…</div>
      )}
      {error && <div className="text-xs text-rose-600">Failed to load: {error}</div>}
      {!loading && !error && docs.length === 0 && (
        <div className="text-xs text-muted-foreground">
          No documents uploaded for this candidate yet.
        </div>
      )}
      ...
      {docs.map((d) => {
        ...
        const extractionVersions =
          Array.from(new Set(fields.map((f) => f.ai_model_version).filter(Boolean))).join(", ") ||
          d.ai_model_version ||
          "—";
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-muted-foreground">
                  <span
                    className={cn(
                      "inline-flex items-center gap-1 rounded px-1.5 py-0.5 ring-1",
                      pill.tone,
                    )}
                  >
                    {pill.icon}
                  ...
                  <span
                    className={cn(
                      "rounded px-1.5 py-0.5 ring-1 uppercase tracking-wider",
                      statePill(d.document_state),
                    )}
                  >
            ...
            {isOpen && (
              <div className="border-t border-border/60 px-3 pb-3 pt-2">
                <dl className="grid grid-cols-[auto,1fr] gap-x-3 gap-y-1 text-muted-foreground">
                  ...
                  <dt>SHA-256</dt>
                  <dd
                    className="flex items-center gap-1 font-mono tabular-nums text-foreground"
                    title={d.sha256 ?? undefined}
                  >
                    {shortHash(d.sha256)} {d.sha256 && <CopyButton value={d.sha256} />}
                  </dd>

                  <dt>Original filename</dt>
                  <dd className="truncate text-foreground" title={d.file_name}>
                    {d.file_name}
                  </dd>
                  ...
                  <dd
                    ...
                    title={d.standardized_filename ?? undefined}
                  >
                  ...
                  <dt>Storage path</dt>
                  <dd
                    className="flex items-center gap-1 truncate font-mono text-foreground"
                  ...
                  <dd
                    className={cn("text-foreground", lowConf > 0 && "font-medium text-amber-700")}
                  >
                    {lowConf}
                  </dd>
                  ...
                  <dd className="text-foreground">
                    {d.verified_at
                      ? `${new Date(d.verified_at).toLocaleString()} · ${d.verified_by ?? "—"}`
                      : "—"}
                  </dd>
